...
function wppb_manage_rf_cpt(){
	...
	$settings_fields = array( 		
		...
		array( 'type' => 'select', 'slug' => 'redirect', 'title' => __( 'Redirect', 'profile-builder' ), 'options' => array( '%'.__('Choose...', 'profile-builder').'%-', '%'.__('No', 'profile-builder').'%No', '%'.__('Yes', 'profile-builder').'%Yes' ), 'default' => '-', 'description' => __( 'Whether to redirect the user to a specific page or not', 'profile-builder' ) ),
	...
	$args = array(
		...
		'meta_array' => $rf_fields = apply_filters	( 'wppb_rf_fields', array( 
																				array( 'type' => 'select', 'slug' => 'field', 'title' => __( 'Field', 'profile-builder' ), 'options' => $rf_fields, 'default-option' => true, 'description' => __( 'Choose one of the supported fields you manage <a href="'.admin_url( 'admin.php?page=manage-fields' ).'">here</a>', 'profile-builder' ) ),
