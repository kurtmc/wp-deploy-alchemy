...
function wppb_manage_epf_cpt(){
	...
	$settings_fields = array( 		
		array( 'type' => 'select', 'slug' => 'redirect', 'title' => __( 'Redirect', 'profile-builder' ), 'options' => array( '%Choose...%-', 'No', 'Yes' ), 'default' => '-', 'description' => __( 'Whether to redirect the user to a specific page or not', 'profile-builder' ) ),
	...
	if ( ( $wppb_manage_fields != 'not_set' ) && ( ( is_array( $wppb_manage_fields ) ) && ( !empty( $wppb_manage_fields ) ) ) ){
		$wppb_epf_unwanted_fields = array( 'reCAPTCHA' );
	...
	$args = array(
		...
		'meta_array' => apply_filters( 'wppb_epf_fields', array( 
																	array( 'type' => 'select', 'slug' => 'field', 'title' => __( 'Field', 'profile-builder' ), 'options' => $epf_fields, 'default-option' => true, 'description' => __( 'Choose one of the supported fields you manage <a href="'.admin_url( 'admin.php?page=manage-fields' ).'">here</a>', 'profile-builder' ) ),
