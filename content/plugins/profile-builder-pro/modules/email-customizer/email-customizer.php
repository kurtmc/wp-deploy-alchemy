...
function wppb_email_customizer_generate_merge_tags( $special_merge_tags = null ){
	...
	$mustache_vars = array(
						...
						array(
							'group-title' => __( 'User Meta', 'profile-builder' ),
...
function wppb_email_customizer_password_reset_title_filter_handler( $default_string, $username ) {
	...
	if( $email_customizer_option != 'not_found' ) {
        ...
        if( !empty( $user_info->data->user_email ) )
            ...
            $user_email = '';
		return (string) new PB_Mustache_Generate_Template( wppb_email_customizer_generate_merge_tags( 'password_reset' ), $email_customizer_option, array( 'user_login' => $username, 'user_email' => $user_email ) );
	}
...
function wppb_email_customizer_password_reset_success_title_filter_handler( $default_string, $username ) {
	...
	if( $email_customizer_option != 'not_found' ) {
        ...
        if( !empty( $user_info->data->user_email ) )
            ...
            $user_email = '';
		wppb_change_email_from_headers();
		return (string) new PB_Mustache_Generate_Template( wppb_email_customizer_generate_merge_tags(), $email_customizer_option, array( 'user_login' => $username, 'user_email' => $user_email ) );
	}
...
function wppb_ec_replace_site_name( $value, $merge_tag_name, $merge_tag, $extra_data ){
	return apply_filters( 'wppb_ec_site_name_filter', html_entity_decode( get_bloginfo( 'name' ) ) );
}
...
function wppb_ec_replace_user_meta( $value, $merge_tag_name, $merge_tag, $extra_data ){
    ...
    return apply_filters( 'wppb_ec_filter_user_meta', $value, $merge_tag_name );
}
add_filter( 'mustache_variable_ec_user_meta', 'wppb_ec_replace_user_meta', 10, 4 );


add_filter( 'wppb_ec_filter_user_meta', 'wppb_ec_change_user_meta_by_type', 10, 2 );
function wppb_ec_change_user_meta_by_type( $value, $meta_name ){
