...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'plugins.php' ) ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
			...
			// Checks for actions from hover links to process the installation.
			elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'update-plugin' == $_GET['tgmpa-install'] ) ) {
				check_admin_referer( 'tgmpa-install' );
				...
				// Prep variables for Plugin_Installer_Skin class.
				$title = sprintf( $this->strings['installing'], $plugin['name'] );
				$url   = esc_url( add_query_arg( array( 'action' => 'update-plugin', 'plugin' => urlencode( $plugin['slug'] ) ), 'update.php' ) );
				if ( isset( $_GET['from'] ) ) {
					$url .= esc_url( add_query_arg( 'from', urlencode( stripslashes( $_GET['from'] ) ), $url ) );
				}

				$nonce = 'update-plugin_' . $plugin['slug'];

				// Prefix a default path to pre-packaged plugins.
				$source = ( 'upload' == $type ) ? $this->default_path . $plugin['source'] : $plugin['source'];

				$names = array( $plugin['name'] );
				$url   = esc_url( add_query_arg( array( 'page' => urlencode( TGM_Plugin_Activation::$instance->menu ) ), self_admin_url( 'plugins.php' ) ) );

				// Create a new instance of Plugin_Upgrader.
				// $upgrader = new Plugin_Upgrader( $skin = new Plugin_Installer_Skin( compact( 'type', 'title', 'url', 'nonce', 'plugin', 'api' ) ) );
				$upgrader = new TGM_Bulk_Installer( $skin = new TGM_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );

				// Perform the action and update the plugin from the $source urldecode().
				$upgrader->bulk_install( array( $source ), true );
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				$show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'plugins.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
				$show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'plugins.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tgmpa' ) . '</a>'  : '';

				if (empty($show_install_link) && $update_link) {
				  $show_install_link  = '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'plugins.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>';
				}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function column_plugin( $item ) {
			...
			$do_update = false;
			$repository_version = TGM_Plugin_Activation::$instance->_get_plugin_data_from_name( $item['sanitized_plugin'], 'version' );
			if ( isset( $installed_plugins[ $item['file_path'] ] ) && $repository_version ) {
				$do_update = version_compare( $installed_plugins[ $item['file_path'] ]['Version'], $repository_version, '<' );
			}
			...
			if ( $do_update ) {
				$actions['update'] = sprintf(
					'<a href="%1$s" title="' . esc_attr__( 'Update', 'tgmpa' ) . ' %2$s">' . __( 'Update', 'tgmpa' ) . '</a>',
					esc_url(
						wp_nonce_url(
							add_query_arg(
								array(
									...
									'plugin_name'   => urlencode( $item['sanitized_plugin'] ),
									'plugin_source' => urlencode( $item['url'] ),
									'tgmpa-install' => urlencode( 'update-plugin' ),
								),
								self_admin_url( 'plugins.php' )
							),
							'tgmpa-install'
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				$url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( TGM_Plugin_Activation::$instance->menu ) ), self_admin_url( 'plugins.php' ) ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;
			...
			if ( 'tgmpa-bulk-update' === $this->current_action() ) {
			  ...
			  echo '<h2>' . esc_html( get_admin_page_title() ) . '</h2>';
			  // Process the bulk installation submissions.
			  $installer->bulk_install( $sources , true);
			  echo '</div>';
	...
	if ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) {
		...
		if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
			...
			class TGM_Bulk_Installer extends WP_Upgrader {
				 ...
				 */
				public function bulk_install( $packages, $update = false ) {
					...
					foreach ( $packages as $plugin ) {
						...
						$result = $this->run(
							array(
								...
								'clear_destination' => $update, // Do we want to clear the destination or not?
								'clear_working'     => true, // Remove original install file.
								'is_multi'          => true, // Are we processing multiple installs?
								'hook_extra'        => array( 'plugin' => $plugin, 'update' => $update), // Pass plugin source as extra data.
							)
						);
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function bulk_footer() {
						...
						foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
							if ( ! is_plugin_active( $plugin['file_path'] ) ) {
								echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( TGM_Plugin_Activation::$instance->menu ), self_admin_url( 'plugins.php' ) ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
								$complete[] = $plugin;
								break;
							}
							// Nothing to store.
						...
						if ( empty( $complete ) ) {
							echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
